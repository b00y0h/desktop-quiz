...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { imageData, imageUrl, answer, pin } = body
    const quiz = store.getQuiz(id)
    if (!quiz) return NextResponse.json({ error: 'Quiz not found' }, { status: 404 })
    if (pin !== quiz.adminPin) return NextResponse.json({ error: 'Invalid PIN' }, { status: 403 })
    if ((!imageData && !imageUrl) || !answer) return NextResponse.json({ error: 'Image and answer required' }, { status: 400 })
    const question = store.addQuestion(id, imageData || imageUrl, answer, imageUrl)
